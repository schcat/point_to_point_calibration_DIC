...
for k=1:1:n
    ...
    imwrite(mat2gray(result), ['../image/num_train_36/simulate_speckle_patent_proj_test_',num2str(k),'.png']);
end
...
for k=1:1:n
    ...
    imwrite(mat2gray(result), ['../image/num_train_36/ROI_proj_test_',num2str(k),'.png']);
end
toc
%DICï¿½ï¿½ï¿½ï¿½
pathname = '../image/num_train_36/';
radius = 70; %DIC subset radius
spacing = 0;
% ï¿½ï¿½ï¿?ROIï¿½ï¿½padï¿½?±ä»¯ï¿½ï¿½ï¿½?µï¿½ï¿½ï¿½ncorr_auto_initseeds.mï¿½ï¿½ï¿½ï¿½ï¿?pad
parpool(36);
mat_name = ['../result/train_num_36/speckle_map_test_'];
parfor k=1:1:n
tic
...
for k = 1:1:n
    str=strcat('../result/train_num_36/speckle_map_test_',num2str(k),'.mat');
    load(str);
...
save(['../result/train_num_36/speckle_map_test_70.mat'],'mean_map_u','mean_map_v');

plot_u = zeros(res_camera_h,res_camera_w);
plot_v = zeros(res_camera_h,res_camera_w);
%pathname = 'image/ori_0315/';
load('../result/train_num_36/speckle_map_test_70.mat');
%imshow(mean_map_u);
...
%����У��
for k=1:1:n
    I = imread(['../image/num_train_36/cali_',num2str(k),'.png']);
...
%    imshow(I_correct/max(I_correct(:)));
    imwrite(I_correct/max(I_correct(:)), ['../image/num_train_36/correct_',num2str(k),'.png']);
end
...
parpool(36);
mat_name = ['../result/train_num_36/speckle_correct_'];
parfor k=1:1:n
tic
    filename_cur = ['num_train_36/correct_',num2str(k),'.png'];
...
fid = fopen('../result/train_num_36/speckle_correct.txt','w');
for k = 1:1:n
    str=strcat('../result/train_num_36/speckle_correct_',num2str(k),'.mat');
    load(str);
...
M=load('board.txt');       %��ȡ��������
M=[M';ones(1,49)];
m_all=load('../result/train_num_36/speckle_correct.txt');       %��ȡ�ؼ�������
m_one=ones(3,49,n);
for i=1:1:n
