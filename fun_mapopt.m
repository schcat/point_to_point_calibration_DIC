...
parfor k=1:1:n
    ...
    imwrite(mat2gray(result), ['../image/train_2_2_p11/simulate_speckle_patent_proj_',num2str(k),'.png']);
end
...
parfor k=1:1:n
    ...
    imwrite(mat2gray(result), ['../image/train_2_2_p11/ROI_proj_',num2str(k),'.png']);
end
%toc
%DIC����
pathname = '../image/train_2_2_p11/';
radius = 60; %DIC subset radius
spacing = 9;
% ���?ROI��pad�б仯���ǵ���ncorr_auto_initseeds.m�����?pad
tic
mat_name = ['../result/train_2_2_70_00625_60/speckle_map_'];
parfor k=1:1:n
...
%tic
for k = 1:1:n
    str=strcat('../result/train_2_2_70_00625_60/speckle_map_',num2str(k),'.mat');
    load(str);
...
for v_loc = 1:1:res_camera_h
    for u_loc = 1:1:res_camera_w
	    ...
	    coef = plot_coef(v_loc, u_loc, k);
            if (u_bias~=0 || v_bias~=0) && coef < 0.0625
...
save(['../result/train_2_2_70_00625_60/speckle_map.mat'],'mean_map_u','mean_map_v');
