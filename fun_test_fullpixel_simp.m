%��ͶӰ���ͼ�?��map������У�����ͽ������ѱ궨���궨
function fun_test_fullpixel(file);
%��ȡmap
...
for k=1:1:n
    ...
    imwrite(mat2gray(result), ['image/ori/train_2/simulate_speckle_patent_proj_test_',num2str(k),'.png']);
end
...
for k=1:1:n
    ...
    imwrite(mat2gray(result), ['image/ori/train_2/ROI_proj_test_',num2str(k),'.png']);
end
toc
%DICï¿½ï¿½ï¿½ï¿½
pathname = 'image/ori/train_2';
radius = 40; %DIC subset radius
spacing = 1;
% ï¿½ï¿½ï¿?ROIï¿½ï¿½padï¿½?±ä»¯ï¿½ï¿½ï¿½?µï¿½ï¿½ï¿½ncorr_auto_initseeds.mï¿½ï¿½ï¿½ï¿½ï¿?pad
parpool(20);
mat_name = ['train_2/speckle_map_test_'];
parfor k=1:1:n
tic
...
for k = 1:1:n
    str=strcat('results/ori_ref_mat/train_2/speckle_map_test_',num2str(k),'.mat');
    load(str);
...
save(['results/speckle_map_test.mat'],'mean_map_u','mean_map_v');
...
parpool(20);
mat_name = ['train_2/speckle_correct_'];
parfor k=1:1:n
tic
    filename_cur = ['ori/train_2/correct_',num2str(k),'.png'];
...
for k = 1:1:n
    str=strcat('results/ori_ref_mat/train_2/speckle_correct_',num2str(k),'.mat');
    load(str);
    for v_loc = 110:10:890
        for u_loc = 110:10:890
...
M=load('board_fullpixel.txt');       %��ȡ��������
M=[M';ones(1,6241)];
m_all=load('speckle_correct.txt');       %��ȡ�ؼ�������
m_one=ones(3,6241,n);
for i=1:1:n
    m_temp = m_all((i-1)*6241+1:i*6241,:);
    m_one(:,:,i) = [m_temp';ones(1,6241)];
end
...
res_sum = zeros(1,12482*n);
for i = 1:n
    for j = 1:6241
        res_sum(1,2*((i-1)*6241+j)-1) = res(1,j,i);
        res_sum(1,2*((i-1)*6241+j)) = res(2,j,i);
    end
end
res_sum.*res_sum;
xxx=sum(res_sum.*res_sum);
initres = sqrt(sum(res_sum.*res_sum,2)/(n*6241))
